...
namespace DkbozkurtPlayableAdsTool.Scripts.PlaygroundConnections
{
    public class TutorialController : SingletonBehaviour<TutorialController>
    {
        ...
        [LunaPlaygroundField("Top Banner Text", 1, "Tutorial Settings")] [SerializeField]
        private string _tutorialTopBannerText;
        ...
        [Header("Tutorial Text Properties")] public GameObject TutorialTextParent;
        public TextMeshProUGUI TutorialText;

        [Header("Tutorial Top Banner Text Properties")]
        public GameObject TutorialTopBannerTextParent;

        public TextMeshProUGUI TutorialTopBannerText;

        [Header("Tutorial Hand Properties")] public GameObject TutorialHandParent;
        public Animator TutorialHandAnimator;

        [Header("Tutorial Arrow Properties")] public Transform TutorialArrowParent;
        [SerializeField] private Transform[] _tutorialArrowWorldSpacePoints;
        ...
        private void OnEnable()
        {
            ...
            TutorialTopBannerTextSetter(true);
            AnimateTutorialText();
        ...
        public void TutorialTopBannerTextSetter(bool status)
        {
            if (_tutorialTopBannerText == "")
            {
                Debug.LogError("Fill Tutorial Top Banner Text !!!");
                return;
            }
            
            if(IsObjectNull(TutorialTopBannerTextParent)) return;
            
            TutorialTopBannerTextParent.SetActive(status);
            
            if(!status) return;

            TutorialTopBannerText.text = _tutorialTopBannerText;
        }
        ...
        {
            if (_tutorialArrowWorldSpacePoints.Length <= 0)
            {
            ...
            TutorialArrowParent.transform.position = _tutorialArrowWorldSpacePoints[index].position;
